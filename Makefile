...
VERSION := $(shell $(PLIST_BUDDY) -c 'Print :version' info.plist)
WORKFLOW_FILE_NAME := $(shell $(PLIST_BUDDY) -c 'Print :name' info.plist)-$(VERSION).alfredworkflow
...
## init: initialize the project from scratch
init: .check-dependencies
	$(if $(shell [[ "" != "$($(PLIST_BUDDY) info.plist -c "Print webaddress")" ]]), , $(shell $(PLIST_BUDDY) info.plist -c "Set webaddress $$(gh repo view --json url -q ".url")"))
	$(if $(shell [[ "" != "$($(PLIST_BUDDY) info.plist -c "Print bundleid")" ]]), , $(shell $(PLIST_BUDDY) info.plist -c "Set bundleid com.grybkov.$$(gh repo view --json name -q ".name")"))
	$(if $(shell [[ "" != "$($(PLIST_BUDDY) info.plist -c "Print name")" ]]), , $(shell $(PLIST_BUDDY) info.plist -c "Set name $(gh repo view --json name -q ".name" | sed -e 's/^alfred-//g')"))
	$(if $(shell [[ "" != "$($(PLIST_BUDDY) info.plist -c "Print description")" ]]), , $(shell $(PLIST_BUDDY) info.plist -c "Set description $$(gh repo view --json description -q ".description")"))
	$(if $(shell [[ "" != "$($(PLIST_BUDDY) info.plist -c "Print version")" ]]), , $(shell $(PLIST_BUDDY) info.plist -c "Set version 1.0.0"))
	$(if $(shell [[ "" != "$($(PLIST_BUDDY) info.plist -c "Print createdby")" ]]), , /usr/libexec/PlistBuddy info.plist -c "Set createdby Illia Grybkov")
	$(if $(shell [[ -f "LICENSE" ]]), , $(shell go run github.com/nishanths/license/v5@latest -o LICENSE mit))

# .refresh-version: refresh version variable
.PHONY: .refresh-version
.refresh-version:
	$(eval VERSION := $(shell $(PLIST_BUDDY) -c 'Print :version' info.plist))
	$(eval WORKFLOW_FILE_NAME := $(shell $(PLIST_BUDDY) -c 'Print :name' info.plist)-$(VERSION).alfredworkflow)
...
set-version:
	@read -p "Enter version [$(VERSION)]: " VERSION \
		...
		&& $(PLIST_BUDDY) -c "Set :version $$VERSION" info.plist && echo "Version updated"

## update-readme: set workflow readme to plist
.PHONY: update-readme
update-readme:
	@$(PLIST_BUDDY) -c "Import :readme README.md" info.plist && echo "Readme updated"
